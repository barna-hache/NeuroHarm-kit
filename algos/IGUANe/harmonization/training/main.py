...
import numpy as np
def convert_to_tfrecord(mri_path, tfrecord_path):
    # Charger l'image MRI
    img = nib.load(mri_path)
    mri_data = img.get_fdata()

    # Normaliser les intensités : cerveau à 0 et arrière-plan à -1
    mri_data = mri_data / 500 - 1

    # Créer l'exemple TFRecord
    with tf.io.TFRecordWriter(tfrecord_path) as writer:
        feature = {
            'mri': tf.train.Feature(float_list=tf.train.FloatList(value=mri_data.flatten()))
        }
        example = tf.train.Example(features=tf.train.Features(feature=feature))
        writer.write(example.SerializeToString())

# Exemple de conversion
convert_to_tfrecord("chemin/vers/votre_image.nii.gz", "chemin/vers/votre_image.tfrecord")
"""
